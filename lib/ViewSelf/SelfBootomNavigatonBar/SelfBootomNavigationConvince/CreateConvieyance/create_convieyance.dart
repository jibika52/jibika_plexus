...
class _CreateConveyanceScreenState extends State<CreateConveyanceScreen> {
  ...
  void initState() {
    Provider.of<TrackingController>(context, listen: false)
        .GetEmployeeLocaltionInfoProvider("${GetStorage().read("mobile_id")}",
            "09-Sep-2024", "60670", context);
    _getCurrentLocation();
  ...
  Widget build(BuildContext context) {
    print(
        "conveyance_car_code--------------------------------------- conveyance_car_code---->  ${GetStorage().read("conveyance_car_code")}");
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
          preferredSize: GetStorage().read("is_Start_Journey") == "true"
              ? Size.fromHeight(1)
              : Size.fromHeight(60),
                  ...
                  Consumer<TrackingController>(
                    builder: (context, value, child) => Container(
                      ...
                      child: ListView.builder(
                          ...
                          // itemCount: 6,
                          itemCount: value.GetVehicleList == null
                              ? 0
                              : value.GetVehicleList.length,
                          itemBuilder: (context, index) => InkWell(
                                onTap: () {
                                  setState(() {
                                    GetStorage().read("select_car_type") == "-1"
                                        ? selected_car_index = index
                                        : selected_car_index = int.parse(
                                            "${GetStorage().read("select_car_type")}");

                                    GetStorage().read("conveyance_car_code") ==
                                            "0"
                                        ? selected_car_code_index = int.parse(
                                            "${value.GetVehicleList[index]["Code"]}")
                                        : selected_car_code_index = int.parse(
                                            "${GetStorage().read("conveyance_car_code")}");
                                  });
                                },
                                child: Container(
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      ColorCustomText(
                                        fontSize: d_height > 700 ? 13 : 11,
                                        fontWeight: FontWeight.w400,
                                        text:
                                            "${value.GetVehicleList[index]["VehicleName"]}",
                                        letterSpacing: 0.3,
                      ...
                      GestureDetector(
                        onLongPress: () => setState(() {
                          ...
                          if (selected_car_index == -1 &&
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                duration: Duration(seconds: 1),
                                content: Text("Enter Fare Amount")));
                          } else {
                            timer = Timer.periodic(Duration(microseconds: 1),
                                (timer) {
                              setState(() {
                                if (increase_punch_progress_bar >= 0.9999999) {
                                  increase_punch_progress_bar = 0.0;
                                  if (GetStorage().read("is_Start_Journey") ==
                                          "false" ||
                                      GetStorage().read("is_Start_Journey") ==
                                          null) {
                                    createConveyance();
                                    GetStorage()
                                        .write("is_Start_Journey", "true");
                                    GetStorage().write("select_car_type",
                                        "$selected_car_index");
                                    GetStorage().write("conveyance_car_code",
                                        "${selected_car_code_index}");
                                    Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (context) =>
                                              SalfBootomNatchBarScreen(
                                                  currentIndex: 3),
                                        ));
                                  } else {
                                    createConveyance();
                                    ...
                                    Future.delayed(
                                      ...
                                      () {
                                        GetStorage()
                                            .write("conveyance_car_code", "0");
                                        GetStorage().write(
                                            "for_end_conveyance_code", "0");
                                        GetStorage()
                                            .write("is_Start_Journey", "false");
                                        GetStorage()
                                            .write("select_conveyance", "");
                                        GetStorage()
                                            .write("select_car_type", "-1");
                                        _amountController.text = "";
                                  ...
                                  increase_punch_progress_bar =
                                      increase_punch_progress_bar + 0.0001;
                                }
                              });
                            });
                          }
                          ...
                          child: Stack(
                            ...
                            children: [
                              Container(
                                ...
                                decoration: GetStorage()
                                                .read("is_Start_Journey") ==
                                            null ||
                                        GetStorage().read("is_Start_Journey") ==
                                            "false"
                                    ? BoxDecoration()
                                    : BoxDecoration(
                                        image: DecorationImage(
                                            image: AssetImage(
                                                "Assets/SelfIcon/jibika_conveyancegiff.gif"))),
                              ...
                              Container(
                                alignment:
                                    Alignment.center, //  color: Colors.green,
                                child: CircularPercentIndicator(
                                  ...
                                  backgroundColor:
                                      Main_Theme_textColor.withOpacity(0.2),
